...
class Board extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.state = {
      ...
      deck: new Array(totalPieces).fill(null).map((_, i: number) => {
        return {
          id: i + 2 * initialHand,
          points: this.set[i + 2 * initialHand]
        };
      }),
      selected: -1,
      center: -1,
  ...
  componentDidUpdate(prevProps: {}, prevState: State) {
    if (prevState.turn !== this.state.turn && this.state.winner === 0) {
      if (
        ...
        const p1 = getPointsSum(this.state.playerOne),
          p2 = getPointsSum(this.state.playerTwo);
  ...
  pass(callback?: () => void) {
    ...
    if (
      this.state.turn === 1 &&
      canPlay(playerOne, this.state.left.value, this.state.right.value)
    ) {
      this.alert('Existe al menos una jugada posible');
      return;
    }
    if (
      this.state.turn === 2 &&
      canPlay(playerTwo, this.state.left.value, this.state.right.value)
    ) {
      this.alert('Existe al menos una jugada posible');
      return;
    }
  ...
  updatePositions(pcs: Array<{ id: number; vertical: boolean }>) {
    ...
    for (let i = pos; i < pieces.length; i++) {
      ...
      if (far <= 175 || far >= this.width() - 100) {
        ...
        dir *= -1;
        if (i + 1 < pieces.length && checkDouble(this.set[pieces[i + 1].id]))
          px += dir * 75;
        else px += dir * 100;
      } else {
        this.positions[i] = {
    ...
    for (let i = pos; pieces.length > 0 && i >= 0; i--) {
      ...
      if (far <= 175 || far >= this.width() - 100) {
        ...
        dir *= -1;
        if (i - 1 >= 0 && checkDouble(this.set[pieces[i - 1].id]))
          px += dir * 75;
        else px += dir * 100;
      } else {
        this.positions[i] = {
  ...
  dragEnd = (e: KonvaEventObject<DragEvent>, x: number, y: number) => {
    ...
    if (pieces.length === 0) {
      ...
      dx1 = nx - this.state.left.x;
      dx2 = nx - this.state.right.x;
      dy1 = ny - this.state.left.y;
      dy2 = ny - this.state.right.y;
      if (
        dx1 * dx1 + dy1 * dy1 < dx2 * dx2 + dy2 * dy2 &&
        dx1 * dx1 + dy1 * dy1 <= 10000
      ) {
        const { value } = this.state.left;
        if (
          value === this.set[selected][0] ||
          value === this.set[selected][1]
        ) {
          this.pushPiece(false, selected);
          placed = true;
        }
  ...
  buttons() {
    ...
    const pass = (
      <PassButton
        ...
        onClick={() => this.pass()}
        disabled={currentPlayer === 'pc' || this.state.winner !== 0}
      />
    );
    let buttons = (
      <>
  ...
  render() {
    ...
    const draw = pieces.map(
      (piece: { id: number; vertical: boolean }, i: number) => {
        return (
          <Piece
            show={true}
            key={piece.id}
    ...
    if (this.state.winner === 0) {
      ...
      messagePoints =
        ...
        ' con ' +
        getPointsSum(
          this.state.winner === 1 ? this.state.playerTwo : this.state.playerOne
        ) +
        ' puntos';
    }
    const { player1, player2 } = this.props;
